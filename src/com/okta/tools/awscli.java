...
/*		String line = oktaBr.readLine();
    ...
    private static String setAWSCredentials(AssumeRoleWithSAMLResult assumeResult, String credentialsProfileName) throws FileNotFoundException, UnsupportedEncodingException, IOException {
        ...
        try {
            if (profilesConfigFile.getCredentials(credentialsProfileName) != null) {


                //if we end up here, it means we were not able to find a matching profile
...
//                pw.println(profileNameLine);
            ...
            //writer.println();
            //writer.println();
            //writer.println(profileNameLine);
            //writer.println("[" + credentialsProfileName + "]");
...
/*
            ...
            //writer.println("[aws-okta]");
            MessageFormat fmt = new MessageFormat("[{0}]");
            writer.println(fmt.format(args));
            //writer.println("[" + credentialsProfileName + "]");
            writer.println("aws_access_key_id=" + awsAccessKey);
            writer.println("aws_secret_access_key=" + awsSecretKey);
            writer.println("aws_session_token=" + awsSessionToken);
 ...
 */
    ...
    private static String totpFactor(JSONObject factor, String stateToken) throws ClientProtocolException, JSONException, IOException{
        ...
        while(sessionToken == ""){
            ...
            sessionToken = verifyAnswer(answer, factor, stateToken, "totp");
        }
        return sessionToken;
    }
